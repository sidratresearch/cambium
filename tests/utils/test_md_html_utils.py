...
from cambium.utils.md_html_utils import (
    _ElementAttributeSet,
    _parse_str_as_attrs,
    markdown_to_html,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("{not attr comment", None),
        ("not attr comment}", None),
        ("{#id-to-use}", _ElementAttributeSet(id="id-to-use")),
        ("{.class-to-use}", _ElementAttributeSet(classes=["class-to-use"])),
        ("{.class-1 .class-2}", _ElementAttributeSet(classes=["class-1", "class-2"])),
        ("{#id .class}", _ElementAttributeSet(id="id", classes=["class"])),
        ("{hidden}", _ElementAttributeSet(simple_attrs=["hidden"])),
        ("{no-quote=head}", _ElementAttributeSet(keyval_attrs=[("no-quote", "head")])),
        (
            '{aria-label="head"}',
            _ElementAttributeSet(keyval_attrs=[("aria-label", '"head"')]),
        ),
        (
            '{key="val" hidden}',
            _ElementAttributeSet(
        ...
        (
            ...
            _ElementAttributeSet(keyval_attrs=[("key", '".val-looks-like-class"')]),
        ),
        # invalid or questionable HTML, do we block?
        ("{#id-with-.}", _ElementAttributeSet(id="id-with-.")),
        (
            "{.0-number-starts-class}",
            _ElementAttributeSet(classes=["0-number-starts-class"]),
        ),
        (
            "{.-hyphen-starts-class}",
            _ElementAttributeSet(classes=["-hyphen-starts-class"]),
        ),
        pytest.param("{#id-1 #id-2}", None, marks=pytest.mark.xfail(reason="2 IDs")),
    ],
)
def test_parse_str_as_attrs(
    comment_string: str, expected: _ElementAttributeSet | None
) -> None:
    """Verify the parsing of comments which add HTML attributes."""
    result = _parse_str_as_attrs(comment_string)
